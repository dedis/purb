...
import (
	...
	"go.dedis.ch/purbs"
	"go.dedis.ch/purbs/experiments-encoding/pgp"
...
func decoding(c *cli.Context) {
	...
	out := SimulMeasureWorstDecodingTime(REPEAT, recipients, suites)
	fmt.Println(out)
}
...
func encoding(c *cli.Context) {
	...
	out := SimulMeasureEncodingTime(REPEAT, recipients, suites)
	fmt.Println(out)
}
...
func headerSize(c *cli.Context) {
	...
	out := SimulMeasureHeaderSize(REPEAT, recipients)
	fmt.Println(out)
}
...
func headerCompactness(c *cli.Context) {
	...
	out := SimulMeasureHeaderCompactness(REPEAT, recipients, suites)
	fmt.Println(out)
}
	...
	for _, nSuites := range suites {
		for _, nRecipients := range recipients {
			for k := 0; k < nRepeat; k++ {
				...
				publicFixedParams := purbs.NewPublicFixedParameters(si, false)
